<template>
    ...
    <section>
        ...
        <li 
          ...
          v-for="comment in sortAuthorComments"
          :key="comment.docID"
        >
          <!-- Comment Author -->
          <div class="mb-5">
...
<script lang="ts">
  ...
  export default {
    setup() {
      ...
      const comment_alert_color = ref<string>('bg-blue-500')
      const comment_alert_message = ref<string>('Please wait! Your comment is being submitted!')
      const comment_in_submission = ref(false)
      const comment_show_alert = ref(false)

      // Populated with data before route enters
      const song = ref<object>({})

      const songComments = ref([])
      // indicate that the order should be - by default - latest to oldest
      const sortComments = ref<string>('1')
      ...
      const addComment = async (values: object, { resetForm }) => {
      ...
      const sortAuthorComments = computed(() => {
        // a new copy of the original array with slice and sorting it
        return songComments.value.slice().sort((a, b) => {
          // if '1' sort in descending order (latest to oldest)
          if (sortComments.value === '1') {
      ...
      watch(() => sortComments.value, (newVal) => {
      ...
      return { 
        n, t, locale, schema, song, newSong, sortComments, 
        comment_in_submission, comment_show_alert, comment_alert_color, comment_alert_message,
        isPlaying, isUserLoggedIn, addComment, getComments, sortAuthorComments,
      }
    ...
    async beforeRouteEnter(to, from, next) {
      ...
      next((vm) => {
        ...
        vm.sortComments = sort === '1' || sort === '2' ? sort : '1' // Checking if the value is valid
